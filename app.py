...
# ---------------------------------------------------------------------------
creds = _creds_available()

with st.sidebar:
    st.header("Run mode")
    default_index = 1 if creds else 0
    mode = st.radio(
    ...
    if live_mode and not creds:
        st.warning(
            "No AWS credentials detected in this environment. Live mode will "
            "fall back to the cached Demo result if the Bedrock call fails."
        )
    st.caption(
        "🔎 Credentials detected: "
        + ("**yes**" if creds else "**no** → defaulting to keyless Demo mode")
    )
...
if not creds:
    st.info(
        ...
        "nothing to break). To run it live against Bedrock, deploy with AWS "
        "credentials or run locally — see the README."
    )
...
# ---------------------------------------------------------------------------
def _run_live(meeting1_text: str, meeting2_text: str) -> dict:
    ...
    if live_mode:
        ...
        try:
            with st.status("Running the two-tool follow-through flow on Amazon Bedrock…", expanded=True) as status:
                ...
                result = _run_live(meeting1, meeting2)
                status.update(label="Live run complete", state="complete", expanded=False)
            _render_results(result, "🟢 Live result — generated just now by the Strands agent via Amazon Bedrock.")
